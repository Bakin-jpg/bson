...
async def scrape_kickass_anime():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # URL target langsung, tidak perlu scrape halaman utama
            ...
            async def is_iframe_valid(iframe_src):
                if not iframe_src or any(s in iframe_src for s in ["Iframe tidak ditemukan", "Iframe tidak tersedia", "Iframe tidak valid"]):
                    return False
                valid_patterns = ["krussdomi.com", "vidstream", "cat-player"]
            ...
            async def get_all_subdub_iframes(watch_page, episode_number):
                ...
                if not available_subdub:
                    print("  Gagal membaca Sub/Dub, mencoba lagi setelah jeda...")
                    await asyncio.sleep(5)
                    available_subdub = await get_available_subdub_from_dropdown(watch_page)

                if not available_subdub:
                    print("  Tetap tidak ada pilihan sub/dub, menggunakan iframe default")
                ...
                for subdub_to_try in available_subdub:
                    ...
                    if await change_subdub_from_dropdown(watch_page, subdub_to_try):
                        ...
                        if await is_iframe_valid(iframe_src):
                            print(f"    ✓ Iframe valid ditemukan untuk {subdub_to_try}")
                            all_iframes[subdub_to_try] = iframe_src
                        else:
                            print(f"    ✗ Iframe tidak valid untuk {subdub_to_try}")
                            all_iframes[subdub_to_try] = "Iframe tidak valid"
                    else:
                        all_iframes[subdub_to_try] = "Gagal ganti Sub/Dub"
                ...
                try:
                    ...
                    if page_dropdown:
                        ...
                        for option in option_elements:
                            ...
                            if text and re.match(r'^\s*(\d+-\d+)\s*$', text.strip()):
                                available_pages.append(text.strip())
            ...
            async def navigate_to_page(watch_page, target_page):
                try:
                    ...
                    for dropdown in all_dropdowns:
                        ...
                        if label_element and "Page" in await label_element.inner_text():
                            page_dropdown = dropdown
                            break
            ...
            async def get_fresh_episode_items(watch_page):
                try:
                    ...
                    return await watch_page.query_selector_all(".episode-item")
                except Exception:
                    return []
                ...
                for page_index, target_page in enumerate(pages_to_scrape):
                    ...
                    for local_ep_index in range(len(episode_items)):
                        ...
                        try:
                            ...
                            episode_items = await get_fresh_episode_items(watch_page) # Refresh
            ...
            try:
                ...
                if not watch_button:
                    raise Exception("Tombol 'Watch Now' tidak ditemukan, tidak bisa melanjutkan.")
                ...
                # --- PERBAIKAN: Tambah wait_until 'networkidle' ---
                await watch_page.goto(first_episode_url, timeout=90000, wait_until="networkidle")
                print("Halaman episode pertama dimuat, menunggu elemen episode list...")
                ...
                # Simpan hasil (bisa di-skip jika hanya ingin lihat log)
                anime_info = {
                    "title": title.strip(), "url_detail": full_detail_url,
                    "total_episodes": page_info["total_episodes"], "episodes": episodes_data,
                    "available_subdub": available_subdub, "has_multiple_pages": page_info["has_multiple_pages"],
                ...
                for i, an in enumerate(existing_data):
                    if an.get('url_detail') == full_detail_url:
                        existing_data[i] = anime_info
                        found = True
                        break
                if not found:
                    existing_data.append(anime_info)
