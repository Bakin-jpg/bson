            ...
            for index, item in enumerate(anime_items[:36]):  # Batasi untuk testing
                ...
                try:
                    ...
                    async def get_available_subdub_from_dropdown(watch_page):
                        ...
                        subdub_options = []
                        try:
                            # Cari dropdown sub/dub yang tepat
                            dropdown_selectors = [
                    ...
                    # Fungsi untuk mendapatkan iframe dengan fallback ke sub/dub lain
                    async def get_iframe_with_fallback(watch_page, episode_number):
                        """Mendapatkan iframe dengan fallback ke sub/dub lain jika iframe default tidak valid"""
                        max_retries = 3
                        
                        for attempt in range(max_retries):
                            try:
                                # Tunggu iframe loading dengan timeout yang lebih lama
                                ...
                                if await is_iframe_valid(iframe_src):
                                    print(f"    ✓ Iframe valid ditemukan (attempt {attempt + 1})")
                                    return iframe_src
                                else:
                                    print(f"    ✗ Iframe tidak valid (attempt {attempt + 1})")
                                    
                                    # Coba sub/dub lain jika iframe tidak valid
                                    if attempt < max_retries - 1:
                                        available_subdub = await get_available_subdub_from_dropdown(watch_page)
                                        if available_subdub and len(available_subdub) > 1:
                                            # Coba sub/dub berikutnya
                                            next_subdub = available_subdub[(attempt + 1) % len(available_subdub)]
                                            print(f"    Mencoba sub/dub alternatif: {next_subdub}")
                                ...
                                print(f"    Gagal mendapatkan iframe (attempt {attempt + 1}): {e}")
                                if attempt < max_retries - 1:
                                    await watch_page.wait_for_timeout(2000)
                    ...
                    async def get_all_subdub_iframes(watch_page, episode_number):
                        ...
                        # Kembali ke subdub original jika ada lebih dari satu opsi
                    ...
                    # Scrape daftar episode dengan iframe masing-masing - PERBAIKAN UTAMA
                    episodes_data = []
                    try:
                        ...
                        # Process episodes - PERBAIKAN: Gunakan pendekatan yang lebih robust
                        for ep_index in range(episodes_to_scrape):
                            try:
                                ...
                                if ep_index >= len(episode_items):
                                    print(f"  Episode index {ep_index} melebihi jumlah episode yang tersedia")
                                    break
                                ...
                                # Klik episode untuk memuat iframe - PERBAIKAN: Gunakan click yang lebih reliable
                                await ep_item.click()
                                
                                # PERBAIKAN PENTING: Tunggu sampai iframe benar-benar dimuat
                                await watch_page.wait_for_timeout(3000)
                                
                                # Tunggu sampai player container ter-refresh
                                try:
                                ...
                                # Coba fallback - ambil iframe tanpa ganti sub/dub
                                try:
                                    fallback_iframe = await get_iframe_with_fallback(watch_page, f"EP {ep_index + 1}")
                                    episodes_data.append({
                                        "episode_number": f"EP {ep_index + 1}",
                                        "episode_url": None,
                                        "iframe_url": fallback_iframe,
                                        "subdub_used": "Fallback",
                                        "status": "fallback",
                                        "all_subdub_iframes": {"Fallback": fallback_iframe}
                                    })
