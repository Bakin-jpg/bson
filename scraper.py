        ...
        try:
            ...
            for index, item in enumerate(anime_items[:36]):
                ...
                try:
                    ...
                    # **PERBAIKAN: Deteksi sub/dub yang tersedia - FILTER MENU NAVIGASI**
                    available_subdub = []
                    optimal_subdub = None
                    ...
                    try:
                        ...
                        if subdub_dropdown:
                            ...
                            # **FILTER: Hanya ambil yang berisi bahasa (SUB/DUB)**
                            navigation_menu = ['Home', 'Trending', 'Schedule', 'Anime', 'Popular Shows', 'Random']
                            for option in subdub_options:
                                option_text = await option.inner_text()
                                if (option_text and option_text.strip() and 
                                    option_text not in navigation_menu and
                                    any(keyword in option_text for keyword in ['SUB', 'DUB', 'Japanese', 'English', 'Spanish', 'Chinese'])):
                            ...
                            if not available_subdub:
                                for option in subdub_options:
                                    option_text = await option.inner_text()
                                    if option_text and option_text.strip() and option_text not in navigation_menu:
                                        available_subdub.append(option_text)
                    ...
                    # **PERBAIKAN: Deteksi page selector - FILTER MENU NAVIGASI**
                    available_pages = []
                    current_page = "01-100"
                    ...
                    try:
                        ...
                        if page_dropdown:
                            ...
                            # **FILTER: Hanya ambil page episode (format angka)**
                            navigation_menu = ['Home', 'Trending', 'Schedule', 'Anime', 'Popular Shows', 'Random']
                            for option in page_options:
                                page_text = await option.inner_text()
                                if (page_text and page_text.strip() and 
                                    page_text not in navigation_menu and
                                    page_text not in available_subdub):  # Exclude sub/dub options
                                    available_pages.append(page_text)
                            
                            print(f"  → Tersedia pages: {available_pages}")
                            
                            # **FILTER LANJUT: Hanya page dengan format angka**
                            filtered_pages = []
                    ...
                    for page_index, target_page in enumerate(available_pages):
                        # **PERBAIKAN: Cek apakah watch_page masih valid**
                        if watch_page is None or watch_page.is_closed():
                            print("  ! Watch page tidak valid, membuat ulang...")
                            watch_page = await context.new_page()
                            await watch_page.goto(first_episode_url, timeout=90000)
                            await watch_page.wait_for_selector(".player-container", timeout=30000)
                        ...
                        if len(available_pages) > 1 and current_page != target_page:
                            ...
                            try:
                                # Pastikan page_dropdown masih ada
                                page_dropdown = await watch_page.query_selector(".v-select:has(.v-label:has-text('Page'))")
                                if not page_dropdown:
                                    print("  ! Page dropdown tidak ditemukan, skip ganti page")
                                    continue
                        ...
                        if episodes_in_current_page == 0:
                            print(f"  ! Tidak ada episode ditemukan di page {current_page}")
                            continue
                        ...
                        for ep_index in range(min(episodes_in_current_page, 10)):  # Batasi per page
                        ...
                        for local_ep_index in episodes_to_scrape_in_page:
                            ...
                            try:
                                ...
                                # **PERBAIKAN: Pastikan watch_page masih valid**
                                if watch_page is None or watch_page.is_closed():
                                    print("  ! Watch page closed, membuat ulang...")
                                    watch_page = await context.new_page()
                                    await watch_page.goto(first_episode_url, timeout=90000)
                                    await watch_page.wait_for_selector(".player-container", timeout=30000)
                                ...
                                print(f"    ✓ Episode {ep_number} berhasil di-scrape (status: {status})")
                    ...
                    # **PERBAIKAN: Pastikan halaman ditutup dengan benar**
                    try:
