...
async def scrape_kickass_anime():
    """
    Scrape data anime lengkap dari kickass-anime.ru dengan DNS AdGuard untuk memblokir iklan.
    """
    async with async_playwright() as p:
        # --- PERBAIKAN: MENAMBAHKAN DNS ADGUARD SAAT LAUNCH ---
        browser = await p.chromium.launch(
            headless=True,
            args=[
                '--host-resolver-rules="MAP * 94.140.14.14"', # Paksa semua request DNS via AdGuard
            ]
        )
        # --- AKHIR PERBAIKAN ---
        ...
        try:
            ...
            print(f"--- MEMULAI MODE DEBUG DENGAN DNS ADGUARD: HANYA MEMPROSES {target_anime_url} ---")
            
            # ... (sisa skrip tidak perlu diubah) ...
            ...
            if os.path.exists('anime_data.json'):
                ...
                except Exception as e: print(f"Error membaca anime_data.json: {e}")
            ...
            async def detect_pages_and_episodes(watch_page):
                ...
                try:
                    ...
                    for dropdown in all_dropdowns:
                        ...
                        if label_el and "Page" in await label_el.inner_text():
                            page_dropdown = dropdown; print("  → Dropdown 'Page' yang benar ditemukan."); break
                    if page_dropdown:
                        await page_dropdown.click(force=True, timeout=5000)
                        await watch_page.wait_for_selector(".v-menu__content--active", timeout=5000)
                        print("  → Menu Page berhasil dibuka.")
                        option_elements = await watch_page.query_selector_all(".v-menu__content--active .v-list-item__title")
                        for option in option_elements:
                            text = await option.inner_text()
                            if text and re.match(r'^\s*(\d+-\d+)\s*$', text.strip()): available_pages.append(text.strip())
                        await watch_page.keyboard.press("Escape")
                        print(f"  → Halaman yang tersedia: {available_pages}")
                    ...
                    await watch_page.keyboard.press("Escape")
                    return { "available_pages": [], "total_episodes": len(await watch_page.query_selector_all(".episode-item")), "has_multiple_pages": False }

            # ... (Semua fungsi helper lainnya tetap sama persis) ...
            async def change_subdub_from_dropdown(watch_page, target_subdub):
                try:
                    dropdown = None; all_dropdowns = await watch_page.query_selector_all(".episode-list .v-select")
                    for d in all_dropdowns:
                        label_el = await d.query_selector(".v-label")
                    ...
                    if target_option:
                        await target_option.click(force=True); await asyncio.sleep(4)
            ...
            async def navigate_to_page(watch_page, target_page):
                try:
                    ...
                    await page_dropdown.click(force=True); await watch_page.wait_for_selector(".v-menu__content--active", timeout=5000)
                    page_option = await watch_page.query_selector(f"//div[contains(@class, 'v-menu__content--active')]//div[contains(text(), '{target_page}')]")
                    if page_option: await page_option.click(force=True); await asyncio.sleep(4); print(f"  ✓ Berhasil ganti ke page: {target_page}"); return True
                ...
                for page_index, target_page in enumerate(pages_to_scrape):
                    ...
                    for local_ep_index in range(len(episode_items)):
                        ...
                        try:
                            ...
                            await ep_item.scroll_into_view_if_needed(); await ep_item.click(force=True, timeout=5000)
                            await watch_page.wait_for_selector("iframe.player[src*='krussdomi']", timeout=20000) # Timeout dinaikkan
                            await asyncio.sleep(3) # Jeda ekstra
            ...
            # ... (Loop utama sama persis) ...
            detail_page = None; watch_page = None
            try:
                ...
                await asyncio.sleep(3) # Jeda krusial agar semua skrip iklan/overlay selesai (atau gagal dimuat)
